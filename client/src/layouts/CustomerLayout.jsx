...
import { customerNavGroups } from '../config/navigation';
...
export const CustomerLayout = () => {
  ...
  const [isCollapsed, setIsCollapsed] = useState(() => {
    return localStorage.getItem('fk_customer_sidebar_collapsed') === 'true';
  });
  ...
  const handleToggleCollapse = () => {
    setIsCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem('fk_customer_sidebar_collapsed', String(next));
      return next;
    });
  };
  ...
  // Derive human-friendly mobile header title from pathname
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col lg:flex-row antialiased">
      {/* 1. Customer Left Sidebar (Collapsible Desktop + Slide Drawer Mobile) */}
      <PortalSidebar
        portal="customer"
        title="FairKart"
